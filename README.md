...
- `hello.gno` — a realm with `Render`, which is what gnoweb displays, plus `Set`
  and `Get`.
- `hello_test.gno` — the two tests every realm wants: one for the state-changing
  function, one that calls `Render`.
